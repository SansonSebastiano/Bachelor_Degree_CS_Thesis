...
Per comprendere al meglio la peculiarità di \emph{Flutter}\cite{site:flutter} descritta nella sezione \ref{subsec:flutter} è necessario analizzare l'architettura\cite{site:flutter-architecture} delle applicazioni realizzate con essa, che sono composte dagli elementi illustrati nella figura \ref{fig:architettura-flutter}, tra i quali, i principali sono descritti di seguito:
\begin{itemize}
...
\emph{Riverpod}\cite{site:riverpod} semplifica notevolmente lo \emph{state management} e si basa su un concetto evoluto da \emph{Provider}\cite{site:provider}, libreria da cui deriva.\\
...
\emph{GitHub}\cite{site:github} è un servizio di hosting per il codice sorgente di progetti software e utilizza \emph{Git}.\\
...
È stato utilizzato per la realizzazione dei casi d'uso (vedi sezione \ref{sec:caso-uso}) e dei diagrammi delle classi.\\
