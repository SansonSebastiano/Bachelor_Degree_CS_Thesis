...
Il file \lstinline{main.dart} è il punto di ingresso dell'applicazione, in particolare è presente la funzione \lstinline{main} che si occupa di inizializzare l'applicazione, fissando l'orientamento dello schermo a \lstinline{portraitUp} e dichiarando il \lstinline{ProviderScope} (vedi sezione \ref{subsec:provider}). \\
...
Per la gestione della responsività dell'applicazione è stata utilizzata la libreria \lstinline{flutter_screenutil}\cite{site:flutter-screenutil} che si occupa di adattare le dimensioni degli elementi grafici in base alle dimensioni dello schermo del dispositivo. \\
In particolare è stato utilizzato il \emph{widget} \lstinline{ScreenUtilInit} che si occupa di inizializzare la libreria, inoltre è stato fissato il parametro \lstinline{designSize} con le dimensioni del \gls{mockup}\glsoccur, in modo tale che la libreria possa effettuare i calcoli necessari per effettuare i dovuti adattamenti. 
